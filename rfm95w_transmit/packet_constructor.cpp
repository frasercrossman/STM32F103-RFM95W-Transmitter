...
void construct_payload(unsigned char* arr, sensors_vec_t* orientation, int time, TinyGPSPlus gps) {
    ...
    construct_imu_segment(&arr[2], orientation);    // Size = 3 (segment header) + 6 * 3  (blocks) = 21 bytes
    construct_time_segment(&arr[23], time);         // Size = 3 (segment header) + 6 * 1  (blocks) =  9 bytes
    construct_gps_segment(&arr[32], gps);           // Size = 3 (segment header) + 10 * 3 (blocks) = 33 bytes
    //construct_battery_segment(&arr[50], 50);
}